...
export default function CasePage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  const humanLabel = (id: string) => labels.find((label) => label.id === id)?.name ?? id.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (char) => char.toUpperCase());

  return (
    <div className="page">
      <header className="page-header">
        <div><div className="eyebrow">Case #{caseId} · Dataset {item.dataset_id}</div><h1>{item.status === "RESOLVED" ? "Resolved" : item.status === "ABSTAINED" ? "Abstained" : item.status === "VOIDED" ? "Voided" : "Awaiting review"}</h1><p>Rubric v{item.rubric_version} is frozen to this case. Validators determine the canonical outcome; similarity only retrieves context.</p></div>
        <div className="button-row"><span className={`status-tag status-${item.status.toLowerCase()}`}>{item.status === "ESCALATED" ? "Awaiting review" : item.status}</span><Link className="quiet-button" href={`/datasets/${item.dataset_id}/disagreements`}>Disputes</Link></div>
      </header>
      <div className="case-layout">
        <section>
          ...
          <div className="case-meta">
            ...
            <div className="meta-cell"><span className="eyebrow">Final label</span><b className={item.final_label ? "signal" : "muted"}>{item.final_label ? humanLabel(item.final_label) : "No canonical label"}</b></div>
            <div className="meta-cell"><span className="eyebrow">Ambiguity</span><b>{item.ambiguity_class || "UNCLASSIFIED"}</b></div>
          </div>
          <div className="section-head"><span>Why it was disputed</span><span>{disagreement.note ?? ""}</span></div>
          <div className="section-body">
            {labels.map((label) => {
              const votes = disagreement.votes?.[label.id] ?? 0;
              return <div className="measure" key={label.id}><span>{label.name}<small className="mono muted"> {label.id}</small></span><span className="measure-track"><span className="measure-fill" style={{ display: "block", width: `${(votes / total) * 100}%` }} /></span><span>{votes}</span></div>;
            })}
          </div>
          <div className="section-head"><span>Related precedent</span><span>Similarity retrieves context</span></div>
          ...
          <div className="precedent-strip">
            {precedents.data.map((precedent) => <article className={`precedent-card ${precedent.same_rubric ? "same" : ""}`} key={precedent.case_id}>
              ...
              <h3>{humanLabel(precedent.final_label)}</h3><div className="mono muted">{precedent.final_label}</div>
          ...
          {item.status !== "ESCALATED" && <details className="receipt"><summary>Technical details</summary>
